...
int main(void)
{
	int(*compare_ptr)(int x, int y);  //declares *compare_ptr function
	int firstArray[8] = { 1,2,3,4,5,6,7,8 };  //sets first array of numbers
	int firstSize = 8;  //size of first array
	int secondArray[5] = { 11,4,9,23,1 };  //sets seconds array of numbers
	int secondSize = 5;  //size of second array

	compare_ptr = &low_to_high_compare;  //sets compare_ptr to the memory address of low_to_high_compare and runs tests below
	printf("First test results in: %d swaps\n", bubble_sort(firstArray, firstSize, compare_ptr));
	printf("Second test results in: %d swaps\n", bubble_sort(secondArray, secondSize, compare_ptr));

	compare_ptr = &high_to_low_compare;  //sets compare_ptr to the memory address of high_to_low_compare and runs tests below
	printf("First test results in: %d swaps\n", bubble_sort(firstArray, firstSize, compare_ptr));
	printf("Second test results in: %d swaps\n", bubble_sort(secondArray, secondSize, compare_ptr));

	compare_ptr = &absolute_high_to_low_compare;  //sets compare_ptr to the memory address of absolute_high_to_low_compare and runs tests below
	printf("First test results in: %d swaps\n", bubble_sort(firstArray, firstSize, compare_ptr));
	printf("Second test results in: %d swaps\n", bubble_sort(secondArray, secondSize, compare_ptr));

	compare_ptr = &absolute_low_to_high_compare;  //sets compare_ptr to the memory address of absolute_low_to_high_compare and runs tests below
	printf("First test results in: %d swaps\n", bubble_sort(firstArray, firstSize, compare_ptr));
	printf("Second test results in: %d swaps\n", bubble_sort(secondArray, secondSize, compare_ptr));
...
int low_to_high_compare(int x, int y)
{
	if (x >= y)  //checks for x greater than or equal to y
	{
		return 1;
	}
	else
...
int high_to_low_compare(int x, int y)
{
	if (x <= y)  //checks for x less than y
	{
		return 1;
	}
	else
...
int absolute_low_to_high_compare(int x, int y)
{
	if (x < 0) 
	{
		x = -x;  //changes negative number to positive
	}

	if (y < 0)
	{
		y = -y;  //changes negative number to positive  
	}
