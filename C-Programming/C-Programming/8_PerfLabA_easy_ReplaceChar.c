...
int replace_char(char * buffer, char findChar, char replaceChar)
{
	...
	if (buffer)
	{
		...
		{
			if (*buffer == findChar)				// Checks for the "findChar" characters
			{
				count++;						// Increment the number of "user inputted" characters removed
				*buffer = replaceChar;					// Changes character to a "replaceChar"
			}
	...
	{
		return ERR_NONE_FOUND;					// Because the function found no "findChar" characters
	}
}
