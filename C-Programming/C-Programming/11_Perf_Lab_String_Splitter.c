...
Address Arithmetic "String Splitter"
...
char * split_the_string(char * string_ptr, char delimiter)
{
	...
	else if (delimiter == '\0')  //checks delimiter for NULL
	{
		return ERROR_NULL_DELIMITER;
	}
	else
	{
		char * returnValue_ptr = string_ptr;  //assigns the pointer value of returnValue_ptr to be the same as string_ptr
		int delimiterNumber = 0;  //starts delimiterNumber (number of counts) to zero
		int i = 0;  //initializes i to zero
		int position = 0; //initializes position to zero

		while (*(string_ptr + i) != '\0')  //starts while loop; runs while the data in the string_ptr index of "i" is not NULL
		{
			if (*(string_ptr + i) == delimiter)  //checks IF the data of string_ptr + i is equal to the delimiter (checks for delimiter in string)
			{
				if (delimiterNumber == 0)  //checks if delimiterNumber is 0
				{
					returnValue_ptr = &*(string_ptr + i + 1);  //sets returnValue_ptr to the memory address of the pointer (string_ptr + i + 1)
					delimiterNumber++;  //increment each time delimiter is found
					position = i;  //sets position to the value of i (index position)
				}
				else  //too many delimiters were found
				{
					return ERROR_ABUNDANT_DELIMITER;
				}
			}
			i++;  //iterate through the loop to the next element/index number
			
		}
		if (delimiterNumber != 0)
		{
			*(string_ptr + position) = '\0';  //inputs a null terminator in the spot the deliminator was found (position is the element that deliminator was found)
		}
