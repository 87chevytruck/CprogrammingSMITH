...
{
	//declare returnValue 
	char * returnValue_ptr = 0;
	int length_sentence_ptr = strlen(*sentence_ptr);
	int length_searchWord_ptr = strlen(*searchWord_ptr);
	int x = 0;
	int y = 0;
	...
	//find the char in searchWord_ptr[0] in the first occurrence of the searchWord_ptr string found in the sentence_ptr string.
	else
	{
		for (x = 0; x <= (length_sentence_ptr - length_searchWord_ptr); x++)
		{
			if (*(sentence_ptr + x) == *(searchWord_ptr))
			{
				returnValue_ptr = *(sentence_ptr + x);
			}
			else
			{
				returnValue_ptr = NULL;
